...
var Bounds = Class.create(
{
    ...
    {   	
		...
		var lastZoom = 1.0;
		var scale = 1.0;
		var bodyZoom = document.body.getStyle("zoom") || 1.0;
		...
		elements.reverse().each(function(element)
		{
			var zoom = element.getStyle("zoom") || 1.0;
			...
			offsetX += element.offsetLeft * zoom;
			offsetY += element.offsetTop * zoom;

			offsetX += element.measure("border-left") * zoom;
			offsetY += element.measure("border-top") * zoom;
		});
		
		this.element.ancestors().reverse().each(function(element)
		{
			if(element != document.body)
			{
				scale *= element.getStyle("zoom") || 1.0;
			}
			...
			if(element.getStyle("position") == "fixed")
			{
				...
				scrollOffsetX = viewportOffset.left * scale;
				scrollOffsetY = viewportOffset.top * scale;
			}
			else
			{
				scrollOffsetX -= element.scrollLeft * scale;
				scrollOffsetY -= element.scrollTop * scale;
			}
		});

		scale *= this.element.getStyle("zoom") || 1.0;
		...
		if(this.boundary == "margin")
		{
			this.left = offsetX - (this.element.measure("margin-left") + this.element.measure("border-left")) * scale;
			this.top = offsetY - (this.element.measure("margin-top") + this.element.measure("border-top")) * scale;
			this.right = this.left + this.element.measure("margin-box-width") * scale;
			this.bottom = this.top + this.element.measure("margin-box-height") * scale;
		}
		else if(this.boundary == "border")
		{
			this.left = offsetX - this.element.measure("border-left") * scale;
			this.top =  offsetY - this.element.measure("border-top") * scale;
			this.right = this.left + this.element.measure("border-box-width") * scale;
			this.bottom = this.top + this.element.measure("border-box-height") * scale;
		}
		else if(this.boundary == "padding")
		{
			this.left = offsetX;
			this.top =  offsetY;
			this.right = this.left + this.element.measure("padding-box-width") * scale;
			this.bottom = this.top + this.element.measure("padding-box-height") * scale;
		}
		else
		{
			this.left = offsetX + this.element.measure("padding-left") * scale;
			this.top =  offsetY + this.element.measure("padding-top") * scale;
			this.right = this.left + this.element.measure("width") * scale;
			this.bottom = this.top + this.element.measure("height") * scale;
		}
	...
	syncElement: function(inElement, inPositions)
	{
		...
		if(this.boundary == "margin")
		{
			left += element.measure("margin-left");
			top += element.measure("margin-top");
			width  -= (element.measure("margin-box-width")  - element.measure("width"));
			height -= (element.measure("margin-box-height") - element.measure("height"));
		}
		else if(this.boundary == "border")
		{
			width  -= (element.measure("border-box-width")  - element.measure("width"));
			height -= (element.measure("border-box-height") - element.measure("height"));
		}
		else if(this.boundary == "padding")
		{
			left -= element.measure("padding-left");
			top -= element.measure("padding-top");
			width  -= (element.measure("padding-box-width")  - element.measure("width"));
			height -= (element.measure("padding-box-height") - element.measure("height"));
		}
		else
		{
			left -= (element.measure("padding-left") + element.measure("border-left"));
			top -= (element.measure("padding-top") + element.measure("border-top"));
		}
	...
	show: function()
	{
		if(!this.showElement)
		{
			this.showElement = new Element("div").setStyle({position:"absolute", backgroundColor:"yellow", opacity:0.2, zIndex:10000, outline:"1px solid red"});
